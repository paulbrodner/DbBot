...
class DatabaseWriter(RobotDatabase):
    ...
    def fetch_id(self, table_name, criteria):
        ...
        res = self._connection.execute(sql_statement, [str(line) for line in criteria.values()]).fetchone()
        if not res:
            raise Exception('Query did not yield id, even though it should have.'
                            '\nSQL statement was:\n%s\nArguments were:\n%s' % (sql_statement, [str(line) for line in criteria.values()]))
        return res[0]

    def insert(self, table_name, criteria):
        sql_statement = self._format_insert_statement(table_name, criteria.keys())
        cursor = self._connection.execute(sql_statement, [str(line) for line in criteria.values()])
    ...
    def insert_or_ignore(self, table_name, criteria):
        ...
        self._connection.execute(sql_statement, [str(line) for line in criteria.values()])
